...
def capture_uart(
    ...
    if mode == "mock":
        lines, passed, note = _simulate_mock_uart(run_id, params)
    ...
    lines, found_end = _read_until_end_marker(port=port, run_id=run_id, timeout_s=timeout_s)
...
def _read_until_end_marker(port: str, run_id: str, timeout_s: float) -> tuple[list[str], bool]:
    deadline = time.monotonic() + timeout_s
